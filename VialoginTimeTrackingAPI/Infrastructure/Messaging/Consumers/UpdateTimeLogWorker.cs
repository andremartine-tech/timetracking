...
{
    public class UpdateTimeLogWorker : BackgroundService
    {
        ...
        public UpdateTimeLogWorker(IServiceProvider serviceProvider, RabbitMqService rabbitMqService, string queueName, int consumerCount = 5)
        {
        ...
        private async void StartConsumer(int consumerId, IChannel channel, CancellationToken stoppingToken)
        {
            ...
            var processMessageUpdateTimeLogUseCase = scope.ServiceProvider.GetRequiredService<IProcessMessageUpdateTimeLogUseCase>();
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                try
                {
                    ...
                    // Processa a mensagem                    
                    await processMessageUpdateTimeLogUseCase.ExecuteAsync(message, stoppingToken);
