...
{
    public class DeleteTimeLogWorker : BackgroundService
    {
        ...
        public DeleteTimeLogWorker(IServiceProvider serviceProvider, RabbitMqService rabbitMqService, string queueName, int consumerCount = 5)
        {
        ...
        private async void StartConsumer(int consumerId, IChannel channel, CancellationToken stoppingToken)
        {
            ...
            var processMessageRegisterTimeLogUseCase = scope.ServiceProvider.GetRequiredService<IProcessMessageDeleteTimeLogUseCase>();
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                try
                {
                    ...
                    // Processa a mensagem                    
                    await processMessageRegisterTimeLogUseCase.ExecuteAsync(message, stoppingToken);
